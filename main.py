...
def guess_letter(word):
    ...
    art.ascii(0)                                    # print welcome text
    art.hangmanpics(guess)                          # print starting
    ...
    while not over:
        ...
        if letter in guessed_letters:                       # check to see if letter has been guessed
            ...
            print("")
            art.hangmanpics(guess)                  # print current gallows
            art.ascii(6)                            # print correct
            ...
            guess += 1
            art.hangmanpics(guess)                  # print current gallows
            art.ascii(5)                            # print incorrect
            if guess > 5:                                   # end if max guesses reached
                streak = 0
                if streak > high_score:
                    high_score = streak
                art.ascii(2)                        # print losing text
                print("It was", word)
                print("High Score: ", high_score)
                over = True
